...
void parserWrite(ArrayList* this, ArrayList* this2)
{
    ...
    if(pFile!=NULL)
    {
        for(i=0;i<al_len(this);i++)
        {
            ...
            if(aux!=NULL)
            {
                fprintf(pFile,"%s,%s,%s,%d,%d,%d\n",aux->nombre,aux->tarjeta,aux->direccion,aux->edad,aux->estado,aux->Id);
            }
        }
...
void parserRead(ArrayList* this, ArrayList* this2)
{
    ...
    if(pFile!=NULL)
    {
        do
        {
            leidos=fscanf(pFile,"%[^,],%[^,],%[^,],%[^,],%[^,],%[^,\n]\n",nombre,tarjeta,direccion,edad,estado,id);
            if(leidos==6)
            {
...
void saveBinary(ArrayList* this)
{
    ...
    ePropietario* aux;
    FILE* pFile;
    pFile=fopen("propietarios.dat","wb");
    if(pFile!=NULL)
    {
        fwrite(this,sizeof(ArrayList),1,pFile);
        for(i=0;i<this->len(this);i++)
        {
            aux=this->get(this,i);
            fwrite(aux,sizeof(ePropietario),1,pFile);
        }
    }
    fclose(pFile);
}
...
void loadBinary(ArrayList* this)
{
    ...
    ePropietario* aux;
    FILE* pFile;
    pFile=fopen("propietarios.dat","rb");
    if(pFile!=NULL)
    {
        fread(this,sizeof(ArrayList),1,pFile);
        for(i=0;i<this->len(this);i++)
        {
            aux=(ePropietario*)malloc(sizeof(ePropietario));
            fread(aux,sizeof(ePropietario),1,pFile);
            this->add(this,aux);
        }
    }
    fclose(pFile);
