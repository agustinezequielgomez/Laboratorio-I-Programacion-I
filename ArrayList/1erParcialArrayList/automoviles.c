...
void mostrarPropietariosQueTenganAudi(ArrayList* this, ArrayList* this2)
{
    int i;
    int j;
    int id;
    ...
    eAutomovil* auxAuto;
    printf("\nPropietarios que tienen coches de la marca AUDI estacionados");
    for(i=0;i<this->len(this);i++)
    {
        auxAuto=this->get(this,i);
        if(auxAuto->marca==3)
        {
            for(j=0;j<this2->len(this2);j++)
            {
                auxProp=this->get(this2,j);
                if(auxAuto->idPropietario==auxProp->Id)
                {
                    printPropietario(auxProp);
                }
            }
        }
    }
...
int comparePatentes(void* elemento1, void* elemento2)
{
    if(strcmp(((eAutomovil*)elemento1)->patente,((eAutomovil*)elemento2)->patente)==1)
    {
        return 1;
    }
    else
...
void mostrarListadoDeAutosPorPatente(ArrayList* this, ArrayList* this2)
{
    int i;
    int j;
    ePropietario* auxProp;
    eAutomovil* auxAuto;
    ArrayList* listaAux;
    listaAux=this->clone(this);
    listaAux->sort(listaAux,comparePatentes,1);
    printf("\nID ingreso\tPatente\tMarca\tID Propietario\tNombre\t\tDireccion\n");
    for(i=0;i<listaAux->len(listaAux);i++)
    {
        auxAuto=listaAux->get(listaAux,i);
        for(j=0;j<this2->len(this2);j++)
        {
            auxProp=this2->get(this2,j);
            if(auxAuto->idPropietario==auxProp->Id)
            {
                printf("%d\t\t%s\t%s\t%d\t\t%s\t\t%s\n",auxAuto->idIngreso,auxAuto->patente,auxAuto->marcaNombre,auxProp->Id,auxProp->nombre,auxProp->direccion);
            }
        }
    }
    system("pause");
